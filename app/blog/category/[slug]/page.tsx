...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!process.env.NEXT_PUBLIC_SANITY_PROJECT_ID) return { title: "Category" }
  ...
  try {
    ...
    const categoryUrl = `${siteConfig.url}/blog/category/${slug}/`

    return {
      title: `${category.title} — Blog`,
      description: `Articles about ${category.title}. Practical tutorials, coding guides, and insights by ${siteConfig.author}.`,
      alternates: {
...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const collectionSchema = getCollectionPageSchema({
    ...
    description: `Articles about ${category.title} by ${siteConfig.author}.`,
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: "Blog", item: "/blog" },
    { name: category.title, item: `/blog/category/${slug}` },
  ])
